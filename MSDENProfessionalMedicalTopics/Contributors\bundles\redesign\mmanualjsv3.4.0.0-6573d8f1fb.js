...
(4881,72-73): run-time error JS1013: Syntax error in regular expression: ,
(4880,70-71): run-time error JS1013: Syntax error in regular expression: ,
 */
var MManual;
(function (MManual) {
    var Foundation;
...
ko.bindingHandlers.stickyHeaderTable = {
    update: function (element, valueAccessor, allBindings, viewModel, bindingContext) {
        var value = ko.unwrap(valueAccessor());
        if (!value) return;
        var fixedTable = $(element).fixedHeadersTable();
        fixedTable.init();
    }
}
...
ko.bindingHandlers.datepicker = {
    init: function (element, valueAccessor, allBindings, viewModel, bindingContext) {
        ...
        var fixPosition = function (input, calendar) {
            calendar.position({
                my: 'left top',
                at: 'right top',
        ...
        if (!options.beforeShow) {
            options.beforeShow = function (input, inst) {
                //datepicker positioning as of https://stackoverflow.com/questions/662220/how-to-change-the-pop-up-position-of-the-jquery-datepicker-control/10598178#10598178
                inputElement = input;
                var calendar = inst.dpDiv;

                setTimeout(function () { fixPosition(input, calendar); }, 0);
                window.addEventListener('resize', function () { fixPosition(input, calendar); });

                $(inputElement).on('focus', blurHandler);
            }
        }

        if (!options.onClose) {
            options.onClose = function (dateText, instance) {
                $(inputElement).unbind('focus', blurHandler);
            }
        }
        ...
        ko.utils.registerEventHandler(element,
            ...
            function () {
                ...
                if (typeof value.changed !== 'undefined') {
                    value.changed(true);
                }
            });

        // prevent focus on input for mobile when clicked icon
...
ko.bindingHandlers.mlinkToolTip = {
    init: function (element, valueAccessor, allBindings, viewModel, bindingContext) {
        ...
        var value = ko.unwrap(valueAccessor());
        var item = ko.mapping.toJS(bindingContext.$data.item);
        var thumbnailUrl = ko.unwrap(value.thumbnail);
        ...
        if (!Modernizr.touchevents) {
            if (showToolTip) {
                $(element).tooltip({
                    content: function () {
                        var mlinkPopOver = "<div class='tooltip-container mlink-tooltip-container'> <a onclick='MLinkpopup(" + (JSON.stringify(item)).replace(/'/g, escape)+ ")' class='anchor'>"
                            + "<div class='tooltip-content'>  <div class='tooltip-heading'>" + title + " </div >"
                            + "<div class='tooltip-img'>"
                            + "<img src='" + thumbnailUrl + "' class='tooltip-Preview' /></div></div></a> </div>"
                        return mlinkPopOver;
                    },
                    open: function (event, ui) {
                        if (typeof (event.originalEvent) === 'undefined') {
...
(function () {
    ...
    var filterTableDefaults = {
        predicate: function () { return true; },
        rowSelector: "tbody tr",
        hideElementSelector: "",
        noMatchingRows: function () { },
        filtered: function (tableId, hasRows) { },
        register: function (tableId) { }
    };

    function getTableRows($table, options) {
        var rows = $table.data("rows");
        if (!rows) {
            rows = $table.find(options.rowSelector);
            $table.data("rows", rows);
        }
        return rows;
    }

    function checkRowsVisibility(rows, options) {
        var rowsVisibility = [];
        rows.each(function (index, row) {
            rowsVisibility[index] = options.predicate(row);
        });
        return rowsVisibility;
    }

    function toggleRowsVisibility(rows, visibility) {
        rows.each(function (index, row) {
            if (visibility[index]) {
                $(row).show();
            } else {
    ...
    function toggleTableVisibility($table, hasMatchingRows, options) {
        var $toggleElement = options.hideElementSelector
            ? $(options.hideElementSelector)
            : $table;
    ...
    function hasMatchingRows(rowsVisibility) {
        var someRowsAreVisible = rowsVisibility
            .some(function (isVisible ) {
                return isVisible;
            });
        return someRowsAreVisible;
    }

    function getTableId($table, options) {
        var $toggleElement = options.hideElementSelector
            ? $(options.hideElementSelector)
            : $table;

        return $toggleElement[0].id;
    }

    function getOptions($table, value) {
        var options = $table.data("options");
        if (!options) {
            options = {};
            $.extend(options, filterTableDefaults, value());
    ...
    ko.bindingHandlers.filterTable = {
        init: function (element, valueAccessor) {
            var $table = $(element);
            var options = getOptions($table, valueAccessor);
            var tableId = getTableId($table, options);
            options.register(tableId);
        },
        update: function (element, valueAccessor, allBindings) {
            var $table = $(element);
            ...
            var rows = getTableRows($table, options);

            var rowsVisibility = checkRowsVisibility(rows, options);

            var hasRows = hasMatchingRows(rowsVisibility);

            toggleTableVisibility($table, hasRows, options);

            if (hasRows) {
                // only make sense to hide rows when table is visible
                toggleRowsVisibility(rows, rowsVisibility);
            }

            var tableId = getTableId($table, options);
            options.filtered(tableId, hasRows);
        }
    }
})();
;
...
function decodeURIComponentSafe(s) {
    ...
    try {
        ...
        console.log("Original text: '" + s + "', escapedText: '" + escapeText + "'");
        console.error(error);
    }
}
    ...
    $(function () {
        ...
        function scrollHandler() {
            ...
            if (window.pageYOffset > fixedOffset) {
                //console.log(window.pageYOffset +'------------------'+ fixedOffset);
...
(function (MManual) {
    ...
    MManual.getTopicsForResource = function (itemId, resultObservableArray, loadingFlagObservable) {
        ...
        var serviceUrl = MManual.settings.topic.getTopicsForResourceUrl;
...
function ShowLexicompMonograph(link) {
    ...
    function getUrl() {
        var controller = '/Redesign/LexicompMonograph/MonographByName';
        var lang = MManual.settings.language;
...
function reassignLink(selector) {
    $(selector)
        .off("click")
        .click(function () {
            ShowLexicompMonograph(this);
...
var checkClickEvent;
function navigateDrugPopup(elem) {
    checkClickEvent = true;
    $(".lexi-navigation-items").find("li").removeClass("active");
    var $elem = $(elem);
    $elem.parent().addClass("active");
    var sectionId = $elem.attr("id").replace("nav_", "");
    var $target = $('.lexi-main p[id="' + sectionId + '"]');
    $target[0].scrollIntoView();
    //$('.lexi-main').scrollTop($target.position().top);
    return false;
}
...
;
(function ($,MManual) {
    $.fn.fixedHeadersTable = function() {
        "use strict";
        var self = this;

        var tableTopOffset              = 0;
        var tableSubheaders             = null;
        var tableSubheadersFixed        = [];
        var tableHeader                 = null;
        var tableHeaderFixed            = null;
        var tableHeaderFixedWrapper     = null;
        var tableHeaderHeight           = null;
        var tableWidth                  = null;
        var table                       = null;
        var wrapper                     = null;
        ...
        function recalculateDimensions() {
            ...
            wrapperWidth      = wrapper.find(".table-box").outerWidth();
            tableHeaderHeight = $(tableHeader).outerHeight();
            tableTopOffset    = wrapper.offset().top;
        }

        function createFixedSubheaders() {
            //if we had tableHead1 we use them if not than tableHead2 if thableHead are siblinks - no fixing.
            if (wrapper.find("tr.tableHead1").length > 1) {
                if (wrapper.find("tr.tableHead1").first().next().hasClass("tableHead1")) {
                    return;
                }
                tableSubheaders = wrapper.find("tr.tableHead1");
            }
            else if (wrapper.find("tr.tableHead2").length > 1) {
                if (wrapper.find("tr.tableHead2").first().next().hasClass("tableHead2")) {
                    return;
                }
                tableSubheaders = wrapper.find("tr.tableHead2");
            }

            //excluding fixing theaders if there are rowspan in headers.
            if (tableSubheaders !== null && tableSubheaders.find("td[rowspan]").length > 0) {
                tableSubheaders = null;
            }

            if (tableSubheaders == null) return;
            tableSubheadersFixed = [];
            tableSubheaders.each(function () {
                var thisSubheader = $(this);

                thisSubheader.find("td").each(function () {
                    $(this).data("cloneorigin", this);
                });

                var subheaderFixed = thisSubheader.clone(true).addClass("fixed");
                tableSubheadersFixed.push(subheaderFixed);
                thisSubheader.after(subheaderFixed);
                thisSubheader.data("fixedclone", subheaderFixed);
                subheaderFixed.css("display", "none");
                thisSubheader.addClass("no-fixed-copy");
            });
            self.off("scroll.stickies").on("scroll.stickies", function () {
                scrollFixedSubheaders();
            });
        }

        function resizeFixedSubheaders() {
            if (tableSubheadersFixed == null) return;

            $.each(tableSubheadersFixed, function () {
                var thisSticky = $(this);

                thisSticky.css("width", wrapperWidth);
                thisSticky.css("min-width", wrapperWidth);
                thisSticky.css('top', tableHeaderHeight + tableTopOffset + 'px');
                thisSticky.find("td").each(function () {
                    var cloneorigin = $(this).data("cloneorigin");
                    $(this).width($(cloneorigin).outerWidth());
                    $(this).css("min-width",$(cloneorigin).outerWidth());
                });
            });
            $.each(tableSubheaders, function () {
                var thisSticky = $(this);
                $.data(thisSticky[0], 'pos', thisSticky.offset().top - table.offset().top + wrapper.offset().top - $(window).scrollTop());
            });

        }

        function scrollFixedSubheaders() {
            if (tableSubheaders == null) return;
            var subheaderToShowIndex = -1;
            tableSubheaders.each(function(i) {
                var thisSubheader = $(this);
                var pos = $.data(thisSubheader[0], 'pos');
                if (pos <= self.scrollTop() + tableHeaderHeight + tableTopOffset) {
                    subheaderToShowIndex = i;
                }
            });
            tableSubheaders.each(function(i) {
                var thisSubheader = $(this);
                var fixedclone = thisSubheader.data("fixedclone");
                if (!fixedclone) return;
                if (i === subheaderToShowIndex) {
                    // do not glue the header if it is too big
                    if ((tableTopOffset + tableHeaderHeight) > window.innerHeight * 0.75) {
                        fixedclone.css("display", "none");
                    } else {
                        fixedclone.css("display", "");
                    }
                } else {
                    fixedclone.css("display", "none");
        ...
        function createFixedHeader() {
            ...
            tableHeaderFixed = tableHeader.clone(true);
            tableHeaderFixedWrapper = $("<table></table>").addClass("fixed-header");
            $(table).after(tableHeaderFixedWrapper);
            tableHeaderFixed.appendTo(tableHeaderFixedWrapper);
            $(tableHeader).addClass("no-fixed-copy");

        }
        function resizeFixedHeader() {
            $(tableHeaderFixed).find("td").each(function () {
                var cloneorigin = $(this).data("cloneorigin");
                $(this).width($(cloneorigin).width());
                $(this).css("max-width", $(cloneorigin).outerWidth());
            });
            $(tableHeaderFixed).find("th").each(function () {
                var cloneorigin = $(this).data("cloneorigin");
                $(this).width($(cloneorigin).width());
                $(this).css("max-width", $(cloneorigin).outerWidth());
            });
            $(tableHeaderFixedWrapper).css("width", wrapperWidth);
            $(tableHeaderFixedWrapper).css("min-width", wrapperWidth);
            $(tableHeaderFixedWrapper).css('top', tableTopOffset + 'px');
            $(tableHeaderFixed).css("width", tableWidth);

        }

        function setHeadersLeftOffset() {
            tableHeaderFixed.css("left", table.offset().left - wrapper.offset().left + "px");
            $.each(tableSubheadersFixed, function() {
                $(this).find("td").each(function() {
                    $(this).css("left", table.offset().left - wrapper.offset().left + "px");
                });
        ...
        function clearHeadersLeftOffset() {
            tableHeaderFixed.css("left", "");
            $.each(tableSubheadersFixed, function() {
                $(this).find("td").each(function() {
                    $(this).css("left", "");
                });
        ...
        function setupHorizontalEvents() {
            table.on('touchmove', function () {
                if (table.offset().left <= 0) {
                    setHeadersLeftOffset();
                }
            });
        ...
        function resizeHeaders() {
            ...
            resizeFixedHeader();
            resizeFixedSubheaders();
            if (window.innerWidth < tableWidth) {
                setHeadersLeftOffset();
            } else {
                clearHeadersLeftOffset();
            }
        }
        ...
        function destroy() {
            ...
            table.off('touchend');
            wrapper.off('scroll');
            window.off('resize', resizeHeaders);
            self.off("scroll.stickies");
            if (tableHeader) {
                tableHeader.find("td").each(function() {
                    $(this).removeData("cloneorigin");
                });
                tableHeader.find("th").each(function () {
                    $(this).removeData("cloneorigin");
                });
            ...
            if (tableSubheaders) {
                $.each(tableSubheaders, function() {
                    var subheader = $(this);
                    subheader.removeData("fixedclone");
                    subheader.find("td").each(function() {
                        $(this).removeData("cloneorigin");
                    });
                });
            }

            if(tableHeaderFixed) $(tableHeaderFixed).remove();
            if(tableHeaderFixedWrapper) $(tableHeaderFixedWrapper).remove();
            if (tableSubheadersFixed) {
                $.each(tableSubheadersFixed, function() { $(this).remove() });
            }
            self.find(".no-fixed-copy").removeClass("no-fixed-copy");
            tableTopOffset       = 0;
            tableSubheaders      = null;
            tableSubheadersFixed = [];
            tableHeader          = null;
            tableHeaderFixed     = null;
...
function MLinkpopup(linkElement) {
    var model = {
        item: ko.mapping.fromJS(linkElement),
    ...
    MManual.Feature.MultimediaPopup.ViewModel.showPopup(model);
...
(function ($, MM) {
    ...
    var overlay = '.breadcrumb__segment--overlay';
    var $dots;
    var $wrapped;
    var $container;
    var $segment;
    ...
    function handleBreadcrumbsMobile() {
        ...
        $container.prepend("<a href='#' class='breadcrumb__segment--overlay' ></a>");
        //$(overlay).hide();
...
;
(function (MManual) {
    MManual.LexicompPlayerDefaultOptions = {
        hideTitle: true,
        hideCarousel: true,
    ...
    MManual.LexicompPlayerViewModel = {
        createViewModel: function (params, componentInfo) {
            var self = this;
            self.parentContext = ko.contextFor(componentInfo.element).$data;

            $.extend(self, params.item);
            $.extend(self, MManual.LexicompPlayerDefaultOptions, params.options);
            self.parentContext.popupTitle(self.genericPopupTitle ? self.MediaNameTranslated : self.title());
            self.parentContext.hideCarousel(self.hideCarousel);
...
ko.components.register('lexicomp-player',
    {
        template: { element: 'lexicomp-player-template' },
...
(function (MManual) {
    ...
    MManual.ResourcePage = {};

    MManual.ResourcePage.ViewModels = {};

    MManual.ResourcePage.ViewModels.Group = function (groupName, items, key) {

        this.sectionName = groupName;
        this.displaySection = ko.observable(true);
        this.displayAllItems = ko.observable(false);
        this.key = key;
        this.sectionImage = null;
        this.showSection = ko.observable(true);

        this.orderItems = function (items) {
            return Enumerable
                .From(items)
                .OrderBy(function (x) { return !!x ? x.Title.toLowerCase() : ""; })
                .ToArray();
        }

        this.items = this.orderItems(items);

        return this;
    }

    MManual.ResourcePage.ViewModels.Builders = {};
    MManual.ResourcePage.ViewModels.Builders.Group = {
        FromSection: function (section) {
            var groupViewModel = new MManual.ResourcePage.ViewModels.Group(
                section.sectionName,
                section.items,
                section.key);
            groupViewModel.sectionImage = section.sectionImage;

            return groupViewModel;
        },
        FromSetting: function(setting, items) {
            var groupViewModel = new MManual.ResourcePage.ViewModels.Group(
                setting,
                items,
                setting);
            groupViewModel.showSection(false);
...
;
(function (MManual) {
    MManual.ResourcePageViewModel = function (mediaServiceUrl, mediaTypes, contentTypes, includedSections, campaignSourceUrlParam, campaignSource, externalPageUrl) {
        var self = this;
        ...
        var settings = window.MediaWidgetSettings;
        var builders = MManual.ResourcePage.ViewModels.Builders;
        ...
        var urlParams = {
            ...
            contentTypes: MManual.utils.getUrlParameter(window.MediaWidgetSettings.urlParams.contentTypes),
            ...
            selectedType: MManual.utils.getUrlParameter(window.MediaWidgetSettings.urlParams.selectedTypeUrlParam)/*video filter change*/
        };
        ...
        self.originalValues = {
            ...
            selectedType: urlParams.selectedType ? urlParams.selectedType.toLowerCase() : null/*video filter change*/
        };
        ...
        self.includedSections = self.originalValues.includedSections;
        ...
        self.selectedType = ko.observable(self.originalValues.selectedType);/*video filter change*/
        ...
        self.displayAllSections = ko.observable(true);
        self.alphabetGroup = ko.observableArray([]);
        self.columnItems = [
            ko.observableArray([]),
            ko.observableArray([]),
            ko.observableArray([])
        ...
        self.parserError = ko.observable(false);
        self.refineBy = ko.observable(false);
        self.goToLetter = ko.observable(false);

        self.pager = ko.observableArray([]);
        self.scrollToPosition = function (data, event) {
            MManual.helpers.setScrollOverride(5);
            $anchor = $(event.target)[0];

            if ((MManual.utils.browser.isIE && location.href == $anchor.href.split('#')[0]) ||
                ((location.pathname.replace(/^\//, '') == $anchor.pathname.replace(/^\//, '') &&
                    location.hostname == $anchor.hostname))
            ) {
                var hashTarget = $($anchor.hash);
                MManual.helpers.expandToDestination(hashTarget);
                MManual.helpers.scrollToDestination(hashTarget);
            }
            // close the opened dropdown-BAETB003-1804
        ...
        //Grid view mode and filter by section
        var viewModeGridAndFilterBySection = function (viewMode) {
            var filterBySection = document.getElementById("sortby-filter") !== null && document.getElementById("sortby-filter").value === window.MediaWidgetSettings.sortOrderBySection;
            return viewMode === window.MediaWidgetSettings.viewModeGrid && filterBySection;
        };

        var pagingUnavalilable = function (vMode) {
            var vm = vMode || self.viewMode();
            return vm === window.MediaWidgetSettings.viewModeSection || vm === window.MediaWidgetSettings.viewModeList || viewModeGridAndFilterBySection(vm);
        };
        ...
        var propagateAllParams = function (uri, isClientSideUpdate, newViewMode, skipPageNumberReset) {
            ...
            propagateParam(uri, isClientSideUpdate, newViewMode, window.MediaWidgetSettings.urlParams.selectedTypeUrlParam, self.selectedType());/*video filter change*/
            if (pagingUnavalilable()) {
                ...
                //load all item if view mode grid and filter by section
                viewModeGridAndFilterBySection(newViewMode) ?
                    propagateParam(uri, isClientSideUpdate, newViewMode, window.MediaWidgetSettings.urlParams.pageSizeUrlParam, window.MediaWidgetSettings.allValue) :
        ...
        self.selectedType.subscribe(function (newValue) {
            ...
            if (evt.type == "change") {
                var param = {};
                param[window.MediaWidgetSettings.urlParams.selectedTypeUrlParam] = newValue;
                MManual.utils.replaceState(param, true);
                $('.resources__refine').removeClass('resources__refine--open');
                if (self.mediaTypes.toString() == "Video" && contentTypes.toString() == "Procedure,Exam") {

                    if (newValue == "") {
                        var urlQueryStr = location.search;
                        var urlVideoRec = document.location.href.replace(document.location.pathname.split("/")[document.location.pathname.split("/").length - 1], "videos");
                        document.location.href = urlVideoRec.replace(urlQueryStr, "?mode=list");
                        return;
                    }
                }
                self.pageNumber(1);
        ...
        self.setSortOrder = function (sortOrder) {
            sortOrder = window.MediaWidgetSettings.getSortOrder(sortOrder);
            ...
            var uri = new URI(window.location.href).search("");
            propagateAllParams(uri, false, self.viewMode(), false);

            window.location.href = uri.href();
        };

        self.showAllItems = function (index) {
            self.pageItems()[index].displayAllItems(!self.pageItems()[index].displayAllItems());
        };

        self.toggleSection = function (index) {
            self.pageItems()[index].displaySection(!self.pageItems()[index].displaySection());
        };

        self.toggleAllSections = function () {
            self.displayAllSections(!self.displayAllSections());
            self.pageItems().forEach(function (section) {
                section.displaySection(self.displayAllSections());
            });
        };

        self.showRefineBy = function () {
            self.goToLetter(false);
            self.refineBy(!self.refineBy());
        };

        self.showGoToLetter = function (id) {
            self.refineBy(false);
            self.goToLetter(!self.goToLetter());
            if (id != null) {
                refreshElement(id);
            }
        };
        ...
        var shouldGroupByAlphabetical = function () {
            return self.sortOrder() === window.MediaWidgetSettings.sortOrderAlphabetical;
        };

        var shouldGroupByCreationDate = function () {
            return self.sortOrder() === window.MediaWidgetSettings.sortOrderByCreationDate;
        }
        ...
        var getSortOrder = function () {
            return self.sortOrder() === "" ? window.MediaWidgetSettings.sortOrderBySection : self.sortOrder();
        }

        var prepareResultsGroupedBySection = function (rawItems, sectionsImageUrl) {
            ...
            for (var i = 0; i < rawItems.length; i++) {
                ...
                if (!item.Sections || item.Sections.length === 0) {
                    ...
                    for (var s = 0; s < item.Sections.length; s++) {
                        ...
                        if (!(self.includedSections.indexOf(section.Key) >= 0)) {
                            continue;
                        }
                        ...
                        if (!groups[section.Key]) {
                            groups[section.Key] = {
                                ...
                                key: section.Key,
                                sectionImage: sectionsImageUrl[section.Key], items: []
                            };
                        }
            ...
            for (var prop in groups) {
                if (groups.hasOwnProperty(prop)) {
                    ...
                    var groupViewModel = builders.Group.FromSection(g);
            ...
            if (itemsWithoutSection.length > 0) {
                var group = builders.Group.FromSetting(settings.noGroup, itemsWithoutSection);
        ...
        var prepareResultsGroupedByAlphabet = function (rawItems) {
            var alphabet = ['#'];
            var showAlphabet = [];

            alphabet = alphabet.concat(MManual.CommonLetters);
            alphabet.forEach(function (letter) {
                var letterEncoded = MManual.utils.encodeText(letter);
                showAlphabet.push({ value: letter, enable: false, key: letterEncoded });
            });
            ...
            for (var i = 0; i < rawItems.length; i++) {
                var item = rawItems[i];
                var firstLetter = MManual.utils.firstLetter(item.Title);

                // encode character to render proper anchors
                // for example  russian letters will be incorrectly encoded
                var firstLetterEncoded = MManual.utils.encodeText(firstLetter);
                ...
                if (!groups[firstLetterEncoded]) {
                    var alphabetIndex = alphabet.indexOf(firstLetter);
                    if (alphabetIndex >= 0) {
                        showAlphabet[alphabetIndex].enable = true;
                    }

                    groups[firstLetterEncoded] = {
                        sectionName: firstLetter,
                        key: firstLetterEncoded,
                        sectionImage: null,
                        items: []
                    };
                }
                groups[firstLetterEncoded].items.push(item);
            }

            self.alphabetGroup(showAlphabet);

            for (var prop in groups) {
                if (groups.hasOwnProperty(prop)) {
                    var g = groups[prop];

                    var group = builders.Group.FromSection(g);
                    result.push(group);
        ...
        var prepareResultsByCreationDate = function (rawItems) {
            var items = [];
            for (var i = 0; i < rawItems.length; i++) {
                var item = rawItems[i];
                items.push(item);
            ...
            self.totalNumberOfItems(rawItems.length);

            var group = builders.Group.FromSetting(settings.sortOrderByCreationDate, items);
        ...
        var prepareResults = function (rawItems) {
            ...
            var group = builders.Group.FromSetting(settings.noGroup, items);
            return [group];
        }
        ...
        var applyClientSideFilters = function () {
            ...
            var items = shouldGroupBySection()
                ...
                : shouldGroupByAlphabetical()
                    ? prepareResultsGroupedByAlphabet(filteredItems)
                    : shouldGroupByCreationDate(filteredItems)
                        ? prepareResultsByCreationDate(filteredItems)
        ...
        var prepareResultItem = function (item) {
            if (self.campaignSource) {
                var uri = null;
                if (item.MediaType === "ExternalPage" || item.MediaType === "LabTest") {
        ...
        self.load = function () {
            self.loading(true);
            var videoContentType = (self.mediaTypes.toString() == "Video" && contentTypes.toString() != "Procedure,Exam") ? self.selectedType() : contentTypes; /*video filter change*/
            
            if (self.mediaTypes.toString() == "Video" && contentTypes.toString() == "Procedure,Exam") {
                videoContentType = (self.selectedType() == "") ? contentTypes : self.selectedType();
            }
            ...
            $.ajax({
                ...
                if (shouldUseClientSideFilters()) {
                    ...
                    var items = shouldGroupBySection()
                        ? prepareResultsGroupedBySection(loadedItems, data.AllSectionsImageUrl)
                        : shouldGroupByAlphabetical()
                            ? prepareResultsGroupedByAlphabet(loadedItems)
                            : shouldGroupByCreationDate(loadedItems)
                                ? prepareResultsByCreationDate(loadedItems)
    ...
    function AdjustRefineByTooltipPositionOnMobile() {
        var goToWidth = $(".resources__goto").outerWidth() + 11;
        $(".resources__refine--tooltip").css('left', '-' + goToWidth + 'px');
    }

    function RestTooltipPosition() {
        $(".resources__refine--tooltip").css('left', 'auto');
    }

    $(function () {
        MManual.Helpers.mq.onBreakPoint(MManual.Helpers.mq.MOBILE, AdjustRefineByTooltipPositionOnMobile, RestTooltipPosition);
    });
}(window.MManual = window.MManual || {}));
;
(function () {
...
(function (MManual) {
    ...
    MManual.RelatedSearchModel = function(query, title) {
        var self = this;
    ...
    MManual.RelatedSearchesViewModel = function (query, searchAddress) {
        var self = this;
            ...
            $.getJSON(self.searchAdress,
                ...
                function(data, status, xhr) {
                    ...
                    for (var i = 1; i < maxIndex; i++) {
                        var rsm = new MManual.RelatedSearchModel(data[i], data[i]);
                        if (i % 2 === 0) {
...
;
(function (MManual) {
    MManual.TablePlayerDefaultOptions = {
        shareUrlBase: 'table/',
        analyticsType: "Tables",
    };
    MManual.TablePlayerViewModel = {
        createViewModel: function (params, componentInfo) {
            var self = this;
            self.parentContext = ko.contextFor(componentInfo.element).$data;
            self.tableBody = ko.observable();
            self.topicsWithResource = ko.observableArray([]);
            self.topicsWithResourceLoading = ko.observable();

            $.extend(self, params.item);
            $.extend(self, MManual.TablePlayerDefaultOptions, params.options);

            $.ajax({
                url: MManual.settings.topic.getTableBodyUrl,
                type: 'GET',
                dataType: "json",
                data: { tableId: self.ItemId },
                async: true,
                success: function (data) {
                    self.tableBody(data.TableBody);

                    MManual.Foundation.Presentation.EventBus.emitEvent(
                        "multimedia.popup.table.show",
                        {
                            itemId: self.ItemId(),
            ...
            if (self.ItemId() && self.loadLocations || self.ItemId()) {
                MManual.getTopicsForResource(self.ItemId(), self.topicsWithResource, self.topicsWithResourceLoading);
            }
            if (self.ShareUrlTitle != null && self.ShareUrlTitle() != null && self.ShareUrlTitle().length > 0) {
                self.parentContext.setShareUrlWithTitle(self.shareUrlBase, self.ShareUrlTitle(), self.Title());
...
ko.components.register('table-player',
    {
        template: { element: 'table-player-template' },
    ...
    MManual.InlinemediaVideoViewModel = {
        createViewModel: function (params, componentInfo) {
            ...
            self.IframeSrc = ko.computed(function() {
                return '//players.brightcove.net/3850378299001/' + self.PlayerId() + '/index.html?videoId=' + self.VideoId();
            });
