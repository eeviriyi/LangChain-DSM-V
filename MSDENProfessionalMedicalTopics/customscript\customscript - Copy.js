...
function search(e) {
    ...
    if (searched.length > 2) {
  	let text = document.getElementById("testone").innerHTML;
	
	 
	 let reone = new RegExp("<mark>","g"); // search for all instances
	 text=text.replace(reone, "");

	  let retwo = new RegExp("</mark>","g");
	text=text.replace(retwo, "");
	
  	let re = new RegExp(searched,"g"); // search for all instances
		let newText = text.replace(re, `<mark>${searched}</mark>`);
	testone.innerHTML = newText;
	document.getElementById("search").value = searched;
  }
}
...
function myFunction() {
  ...
  if (x.style.display === "none") {
	...
	$("#button").removeClass("icon-customcontrol fa-magnifysearch");
	 $("#button").addClass("icon-customcontrol fa-close"); 
