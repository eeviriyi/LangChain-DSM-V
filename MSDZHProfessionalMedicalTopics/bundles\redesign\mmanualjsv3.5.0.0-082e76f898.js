...
(4916,72-73): run-time error JS1013: Syntax error in regular expression: ,
(4915,70-71): run-time error JS1013: Syntax error in regular expression: ,
 */
var MManual;
(function (MManual) {
    var Foundation;
...
function reassignLink(selector) {
    ...
    $(selector).parent().keypress(function (event) {
        var keyCode = (event.which ? event.which : event.keyCode);
        if (keyCode === 13) {          
            ShowLexicompMonograph(this);
        }
...
function MLinkpopup(linkElement) {
    var model = {
        ...
        options: {
            ...
            PopupTitle: linkElement.MediaNameTranslated
        }
    }
